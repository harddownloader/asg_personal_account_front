...
export const AddCargoDialog = observer(
  ({ isVisible, handleCancel, clientCode, clientId }: AddCargoDialogProps) => {
    const {
    ...
    const currentTmpSpaces = useMemo(
      () => getCurrentNewTmpSpaces(JSON.parse(notLoadedSpacesSrt)),
      [notLoadedSpacesSrt]
    )
    ...
    const clearSpacesForAddCargo = () => {
      ...
      const onlyExistsSpaces = tmpSpaces.filter((space: TSpaceItem): boolean => {
        return space.clientId === clientId && space?.cargoId !== undefined
      })
    ...
    return (
      ...
      >
        <CargosForm
          ...
          handleSubmit={handleSubmitForm(handleAddCargo)}
          formControl={{
            registerForm,
            errorsForm,
            setErrorForm,
